...
from .matrix import *
import simulations.models as models
...
def extract_control_descriptor(component_objects, components_found, control_filename, control_files, sim_case, control_id):
    """
    This function reads a control descriptor spreadsheet and
    writes the control function parameters into the database.
    """
    ...
    control_desc_handles = open(control_files,"r")
    params_from_file = reading_params(control_desc_handles)

    for c2 in range(len(params_from_file)):
        # Scrubbing blank spaces from the beginning
        # and the end of the first cell.
        params_from_file[c2][0] = params_from_file[c2][0].strip()
        ...
        if params_from_file[c2][0].lower()=="input":
            ...
            #  it will be a meter.
                ...
                for c3 in range(len(components_found[meter_type.split("_")[0].lower()])):
                    if components_found[meter_type.split("_")[0].lower()][c3][1]==meter_type.split("_")[1]:
                        meter_type_ref = meter_type.split("_")[0].lower()
                        control_file_inputs[components_found[meter_type_ref][c3][0]]=[components_found[meter_type_ref][c3][1]]
                        ...
                        control_file_inputs[components_found[meter_type_ref][c3][0]].append(var_name)
        ...
        if params_from_file[c2][0].lower()=="output":
            ...
            for c3 in range(len(components_found[element_type.split("_")[0].lower()])):
                if components_found[element_type.split("_")[0].lower()][c3][1]==element_type.split("_")[1]:
                    ...
                    # Since a controlled element can have more than one control input
                    # Check if it has been found before.
                    if not components_found[element_type_ref][c3][0] in control_file_outputs.keys():
                        control_file_outputs[components_found[element_type_ref][c3][0]] = [components_found[element_type_ref][c3][1]]

                    control_tag_name = params_from_file[c2][2].split("=")[1]
                    control_var_name = params_from_file[c2][3].split("=")[1]
                    control_init_value = params_from_file[c2][4].split("=")[1]

                    control_tag_name = control_tag_name.strip()
                    control_var_name = control_var_name.strip()
                    control_init_value = control_init_value.strip()

                    control_file_outputs[components_found[element_type_ref][c3][0]].append([control_tag_name, control_var_name, \
                                        float(control_init_value)])
        ...
        if params_from_file[c2][0].lower()=="staticvariable":
            ...
            # will be the variable name.
            staticvar_type = params_from_file[c2][1].split("=")[1]
            staticvar_type = staticvar_type.strip()

            staticvar_val = params_from_file[c2][2].split("=")[1]
            staticvar_val = staticvar_val.strip()
            control_file_staticvars[staticvar_type]=float(staticvar_val)
        ...
        if params_from_file[c2][0].lower()=="timeevent":
            ...
            # will be the variable name.
            timeevent_type = params_from_file[c2][1].split("=")[1]
            timeevent_type = timeevent_type.strip()

            timeevent_val = params_from_file[c2][2].split("=")[1]
            timeevent_val = timeevent_val.strip()

            control_file_timeevents[timeevent_type] = float(timeevent_val)
        ...
        if params_from_file[c2][0].lower()=="variablestorage":
            ...
            # will be the variable name.
            varstore_name = params_from_file[c2][1].split("=")[1]
            varstore_name = varstore_name.strip()

            varstore_val = params_from_file[c2][2].split("=")[1]
            varstore_val = varstore_val.strip()
            ...
            varstore_plot = params_from_file[c2][3].split("=")[1]
            varstore_plot = varstore_plot.strip()
    ...
    # Delete the control variables associated with the control file.
    # Once the upload option is chosen, previous variables in the
    # control file must be purged.
    control_file_object = models.ControlFile.objects.get(id=control_id)

    old_control_inputs = control_file_object.controlinputs_set.all()
    for old_input_item in old_control_inputs:
        old_input_item.delete()

    old_control_outputs = control_file_object.controloutputs_set.all()
    for old_output_item in old_control_outputs:
        old_output_item.delete()

    old_control_statvar = control_file_object.controlstaticvariable_set.all()
    for old_statvar_item in old_control_statvar:
        old_statvar_item.delete()

    old_control_tevents = control_file_object.controltimeevent_set.all()
    for old_tevent_item in old_control_tevents:
        old_tevent_item.delete()

    old_varstores = models.ControlVariableStorage.objects.filter(sim_case=sim_case, \
                    control_file_name=control_file_object.control_file_name)

    for old_varstore_item in old_varstores:
        old_varstore_item.delete()
    control_file_object.save()
    sim_case.save()

    # Write new variables extracted from spreadsheet into the database.
    for inp_key, inp_value in control_file_inputs.items():
        inp_source = component_objects[inp_key].type + '_' + \
                inp_value[0]
        inp_variable_name = inp_value[1]
        inp_object = models.ControlInputs()
        inp_object.input_source = inp_source
        inp_object.input_variable_name = inp_variable_name
        inp_object.control_file = control_file_object
        inp_object.save()
        control_file_object.save()
        sim_case.save()

    for out_key, out_value in control_file_outputs.items():
        out_target = component_objects[out_key].type + '_' + \
                out_value[0]
        out_variable_name = out_value[1][1]
        out_initial_value = out_value[1][2]
        out_object = models.ControlOutputs()
        out_object.output_target = out_target
        out_object.output_variable_name = out_variable_name
        out_object.output_initial_value = out_initial_value
        out_object.control_file = control_file_object
        out_object.save()
        control_file_object.save()
        sim_case.save()

    for static_key, static_value in control_file_staticvars.items():
        static_variable_name = static_key
        static_initial_value = static_value
        statvar_object = models.ControlStaticVariable()
        statvar_object.static_variable_name = static_variable_name
        statvar_object.static_initial_value = static_initial_value
        statvar_object.control_file = control_file_object
        statvar_object.save()
        control_file_object.save()
        sim_case.save()

    for tevent_key, tevent_value in control_file_timeevents.items():
        time_event_name = tevent_key
        time_initial_value = tevent_value
        tevent_object = models.ControlTimeEvent()
        tevent_object.time_event_name = time_event_name
        tevent_object.initial_time_value = time_initial_value
        tevent_object.control_file = control_file_object
        tevent_object.save()
        control_file_object.save()
        sim_case.save()

    for varstore_key, varstore_value in control_file_variablestorage.items():
        var_store_name = varstore_key
        var_store_initial = varstore_value[0]
        var_store_status = varstore_value[1]
        varstore_object = models.ControlVariableStorage()
        varstore_object.variable_storage_name = var_store_name
        varstore_object.storage_initial_value = var_store_initial
        if (var_store_status.lower()=='yes' or var_store_status.lower()=='y'):
            varstore_object.storage_status = 'Y'
        else:
            varstore_object.storage_status = 'N'
        varstore_object.control_file_name = control_file_object.control_file_name
        varstore_object.sim_case = sim_case
        varstore_object.save()
        sim_case.save()

    control_desc_handles.close()

    return



def export_to_control_descriptor(component_objects, components_found, control_filename, control_files, sim_case, control_id):
    """
    This function reads the database and writes control variables
    into a descriptor spreadsheet.
    """

    # If the file is already open, it can't be created.
    # So send an error message asking to close the file.
    try:
        control_desc_handles = open(control_files,"w")
    except:
        return 'Cannot create file. Make sure file with same name is not open.'

    control_file_obj = models.ControlFile.objects.get(id=control_id)

    try:
        control_inputs = control_file_obj.controlinputs_set.all()
    except:
        control_inputs = []

    try:
        control_outputs = control_file_obj.controloutputs_set.all()
    except:
        control_outputs = []

    try:
        control_static_vars = control_file_obj.controlstaticvariable_set.all()
    except:
        control_static_vars = []

    try:
        control_time_events = control_file_obj.controltimeevent_set.all()
    except:
        control_time_events = []

    try:
        control_var_store = sim_case.controlvariablestorage_set.filter(
                control_file_name=control_filename.split('_desc')[0]+'.py')
    except:
        control_var_store = []
    ...
    for output_items in control_outputs:
        print(output_items.output_target, output_items.output_variable_name, str(output_items.output_initial_value))
        comp_items_of_type = components_found[output_items.output_target.split('_')[0].lower()]
        for comp_item in comp_items_of_type:
            if (comp_item[1]==output_items.output_target.split('_')[-1]):
                comp_control_tag = component_objects[comp_item[0]].control_tag[0]
                print(comp_control_tag)
    ...
    for c2 in range(len(params_from_file)):
        ...
        if params_from_file[c2][0].lower()=="input":
            ...
            #  it will be a meter.
                ...
                # The cell position which is unique will be the
                # dictionary key for control_file_inputs.
                if meter_type.split("_")[0].lower() in components_found:
                    for c3 in range(len(components_found[meter_type.split("_")[0].lower()])):
                        if components_found[meter_type.split("_")[0].lower()][c3][1]==meter_type.split("_")[1]:
                            meter_type_ref = meter_type.split("_")[0].lower()
                            control_file_inputs[components_found[meter_type_ref][c3][0]]=[components_found[meter_type_ref][c3][1]]
        ...
        if params_from_file[c2][0].lower()=="output":
            ...
            # and get the cell position from the device tag.
            # The cell position will be the unique dictionary key
            if element_type.split("_")[0].lower() in components_found:
                for c3 in range(len(components_found[element_type.split("_")[0].lower()])):
                    if components_found[element_type.split("_")[0].lower()][c3][1]==element_type.split("_")[1]:
                        element_type_ref = element_type.split("_")[0].lower()
                        # Since a controlled element can have more than one control input
...
def export_to_control_descriptor(component_objects, components_found, control_filename, control_files, sim_case
, control_id):
    """
    This function reads the database and writes control variables
    into a descriptor spreadsheet.
