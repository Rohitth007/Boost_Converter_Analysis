def gate_signal_func(interface_inputs, interface_outputs, interface_static, interface_time, interface_variablestore,                                 interface_events, circuit_components, pos, t_clock, sys_t_events):
	...
	s1_gate=interface_outputs[pos]['1C0'][1][2]
	# Switching Frequency
	sw_freq = 10000.0
	sw_period = 1/sw_freq
	...
	if not interface_outputs[pos]['1C0'][1][2]==s1_gate:
		interface_events[pos] = 1

	circuit_components['1C0'].control_values[0]=s1_gate
	interface_outputs[pos]['1C0'][1][2]=s1_gate
